      ...
      try {
        ...
        await Promise.allSettled([
          read('/api/advertising/report?'+new URLSearchParams({scope:currentScope,startDate:r.startDate,endDate:r.endDate})).then(response=>{
            ...
            root.querySelector('[data-ad-report]').innerHTML=rows.length?`<canvas class="aa-chart" aria-label="当前账号每日广告花费与询盘"></canvas><p class="aa-muted">橙色：花费　蓝色：询盘</p>`:empty('平台在所选区间未返回广告记录，指标保持缺失。');
