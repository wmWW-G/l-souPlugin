...
按钮使用普通英文指令 `Use lsou-launchpad skill; action=<action>`，按本技能名称和参数执行；用户的中文请求同样按原意处理。命令只是入口，状态说明仍用简短中文。

| action | 执行含义 |
|---|---|
| `start` | 用户已点击启动，直接进入下文启动流程，不再显示启动页。 |
| `status` | 查看当前启动状态。 |
| `shutdown` | 按下文规则停止整个插件。 |
| `update` | 按下文规则停止整个插件，再说明同平台新版 ZIP 的导入方式与历史保留；不自行安装或删除文件。 |
