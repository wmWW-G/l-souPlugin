...
test('完整保存零值/缺项并剔除凭据；消息只引用本机文件，同一次点击只唤起一次',async t=>{
 ...
 const url=new URL(urls[0]);assert.match(url.searchParams.get('query'),/^Use lsou-market-positioning skill; input=/);assert.equal(url.protocol,'accio:');assert.equal(url.hostname,'chat');assert.equal(url.pathname,'/new');assert.ok(url.searchParams.get('query').includes(file));assert.ok(!url.href.includes('full keyword'));assert.equal(fs.statSync(file).mode&0o777,0o600);
