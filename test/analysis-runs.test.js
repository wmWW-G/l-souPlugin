...
test('all 31 report routes copy their own skill, preserve context and import reports exactly once', async t => {
  ...
  for (const item of catalog) {
    ...
    const query = h.calls.at(-1).searchParams.get('query'), [skillName, ...args] = query.split('; ');
    const parameters = Object.fromEntries(args.map(argument => argument.split('=')));
    assert.equal(skillName, 'Use ' + item.skill + ' skill');
    ...
    assert.equal(parameters['request'], file.request_ref);
    assert.equal(path.join(h.root, ...parameters['request'].split('/'), 'report.html'), file.result_path);
    assert.match(query,/^[\x20-\x7e]+$/);assert.ok(!query.includes('$'));
  assert.ok(query.length < 160, '消息只携带技能与必要参数，不再展开执行规则');
    // 模拟技能规定的原子回传：临时文件不能提前导入，正式文件就绪后才接收。
    fs.writeFileSync(h.output(id) + '.tmp', report(item.topic));
    assert.equal(h.store.list().requests.at(-1).status, 'pending');
    fs.renameSync(h.output(id) + '.tmp', h.output(id));
