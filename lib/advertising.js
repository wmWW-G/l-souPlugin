...
function unwrap(run) {
  if(String(run.parsed?.error?.message||'').includes('campaign_forbidden_keyword only supports standard promotion campaigns'))throw new Error('平台暂不支持此类计划的屏蔽词查询，请切换标准推广计划');
...
function createAdvertising({ runWorkctl, pushLog = () => {} }) {
  ...
  // 每个详情入口只允许跟随平台给出的同名只读实体，浏览器不能提交任意实体或 SQL。
  const detailEntities={products:'campaign_product',keywords:'campaign_keyword',tags:'campaign_targeting_tag',autoSelect:'campaign_auto_select_product',searchKeywords:'campaign_search_keyword',forbiddenProducts:'campaign_forbidden_product',forbiddenKeywords:'campaign_forbidden_keyword',regionOptions:'region_options',languageSiteOptions:'language_site_options',tagRecommendation:'tag_recommendation'};
  /** 读取真实计划详情及可用导航；id为数字计划ID，返回原始只读包络；失败抛错。 */
  function campaign(id){return memo('campaign:'+id,()=>query({entityType:'campaign',filters:{campaignId:Number(id)},include:'all'}));}
  /** 清除平台写操作和内部调用结构，保留未知业务字段供界面展示；参数为JSON值，返回副本。 */
  function business(value){if(Array.isArray(value))return value.map(business);if(value&&typeof value==='object')return Object.fromEntries(Object.entries(value).filter(([k])=>!['operations','invoke','handoff','links','sessionId','tableName'].includes(k)).map(([k,v])=>[k,business(v)]));return value;}
  ...
  function report(scope, range, campaignId) {
    const task = reportQueue.catch(() => {}).then(async () => {
      let filters={datasource:`company_${scope}`};
      if(campaignId){const d=await campaign(campaignId),link=d.links?.find(l=>l.name==='report'&&l.available&&l.invoke?.entityType==='report');if(!link||String(link.invoke.filters?.campaignId)!==String(campaignId))throw new Error('当前计划未开放效果报告');filters={...link.invoke.filters};}
      const producer = await query({ entityType:'report', include:'data', filters:{...filters,tempTableName:'LSOU_AD_'+require('node:crypto').randomBytes(6).toString('hex').toUpperCase(),beginDateTime:range.startDate+' 00:00:00',endDateTime:range.endDate+' 23:59:59'} });
      const meta = producer.data?.[0];
      const link = producer.links?.find(l => l.available && l.invoke?.entityType === 'report_sql');
      if (!link || meta?.datasource !== filters.datasource) throw new Error('平台未返回匹配的广告报表入口');
      const p = structuredClone(link.invoke);
      if (String(p.filters?.sessionId) !== String(meta.sessionId) || p.filters?.tableName !== meta.tableName || !/^SELECT \* FROM [A-Z_][A-Z0-9_]* LIMIT \d+$/i.test(p.filters?.sql1 || '')) throw new Error('平台报表查询契约变化');
  ...
  async function read(name, q = {}) {
    const allowed = { detail:['id','section','page','keyword','view','refresh'], explanation:['question','refresh'], 'plan-diagnosis':['id','endDate','question','refresh'], plans:['kind','page','refresh'], finance:['refresh'], industry:['refresh'], recommendation:['refresh'], diagnosis:['endDate','refresh'], report:['scope','id','startDate','endDate','refresh'], history:['account','type','startDate','endDate','page','refresh'] };
    ...
    if(['diagnosis','plan-diagnosis'].includes(name)) Time.range(q.endDate,q.endDate,{latest:Time.shift(Time.today(),-1)});
    if((['detail','plan-diagnosis'].includes(name)||(name==='report'&&q.id))&&(!/^\d{1,18}$/.test(String(q.id||''))||!Number.isSafeInteger(Number(q.id))||Number(q.id)<1))throw new Error('请选择有效计划');
    if(name==='detail'&&q.section&&q.section!=='summary'&&!Object.hasOwn(detailEntities,q.section))throw new Error('不支持的计划详情');
    if(q.view&&!['demoted'].includes(q.view))throw new Error('搜索词视图无效');
    if(q.keyword&&q.keyword.length>200)throw new Error('搜索内容过长');
    if(['explanation','plan-diagnosis'].includes(name)&&(!q.question?.trim()||q.question.length>500))throw new Error('请填写500字以内的问题');
    const params = {...q}; delete params.refresh;
    const key = name + JSON.stringify(params);
    return memo(key, async () => {
      if(name==='explanation'){
        const company=await root(),link=company.links?.find(l=>l.name==='explain'&&l.available&&l.invoke?.entityType==='campaign_explanation');
        if(!link)throw new Error('当前账号未开放广告知识问答');
        const d=await query({...link.invoke,include:'data',filters:{query:q.question.trim()}});return {rows:business(d.data||[])};
      }
      if(name==='detail'||name==='plan-diagnosis'){
        const d=await campaign(q.id),section=q.section||'summary';
        if(name==='detail'&&section==='summary')return {rows:business(d.data||[]),sections:Object.keys(detailEntities).map(key=>{const link=d.links?.find(l=>l.name===key);return {key,available:!!(link?.available&&link.invoke),reason:link?.reason||'平台未开放此项'};})};
        const key=name==='plan-diagnosis'?'diagnose':section;
        const link=d.links?.find(l=>l.name===key&&l.available&&l.invoke);
        if(!link)return {rows:[],unavailable:true,reason:d.links?.find(l=>l.name===key)?.reason||'当前计划不支持此项'};
        const expected=name==='plan-diagnosis'?'diagnosis':detailEntities[key];
        if(link.invoke.entityType!==expected)throw new Error('计划详情入口契约变化，请更新适配');
        const filters={...link.invoke.filters};
        if(filters.campaignId!=null&&String(filters.campaignId)!==String(q.id))throw new Error('计划入口归属不一致');
        if(name==='plan-diagnosis')Object.assign(filters,{endDate:q.endDate,question:q.question});
        if(q.keyword&&['products','keywords'].includes(section))filters[section==='products'?'subject':'keyword']=q.keyword;
        if(section==='searchKeywords'&&q.view)filters.view=q.view;
        const result=await query({...link.invoke,include:'data',filters,page:{index:page,size:20}});
        return {rows:business(result.data||[]),total:numeric(result.total),page,metadata:business(result.metadata||{})};
      }
      if (name==='report') return report(q.scope,range,q.id);
      if (name==='plans') {
        const d = await follow(kindMap[q.kind],{}, {index:page,size:50});
        const keys = ['campaignId','campaignName','campaignTypeDesc','campaignType','subType','onlineStatus','budget','onlineProductCount','gmtCreate','gmtModify','gmtModified','optimizeTarget','campaignGroupId','campaignGroupName','campaignGroupType','effectiveStatusLabel','statusLabel','marketingPackageTypeLabel','optimizerTargetLabel','promotionRegions','multipleSites','channelList','matchType'];
