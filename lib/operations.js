...
const READS = {
  ...
  stars: ['other icbu-starrating-cgs-pc-page-data-open', '星等级与提升建议'],
};
const WRITES = {
...
function createOperations({ runWorkctl, callEndpoint, pushLog }) {
  ...
  async function invoke(command, params = {}, write = false) {
    ...
    try {
      ...
      const result = await runWorkctl(['icbu', ...command.split(' '), '--json-file', file,
        ...(write || contract.requiresYes ? ['--yes'] : []), '--format', 'json', '--compact-output', 'off'], write ? 90000 : 35000);
  ...
  async function read(action, source = {}) {
    if (action === 'products') return products(source);
    ...
    if (['messages', 'buyer-basic'].includes(action)) {
      ...
      if (action === 'messages') {
        ...
        params.limitTimeStamp ??= Date.now(); params.domain = 'icbu'; params.count = 30; params.forward = false;
      } else {
    ...
    const data = await invoke(READS[action][0], params);
  ...
  async function handle(req, res, url) {
    ...
    try {
      ...
      if (url.pathname === '/api/operations/read' && req.method === 'POST') {
        const input = await body(req); const data = await read(input.action, input.params);
