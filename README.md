...
按钮交给 Accio Work 的消息只保留 英文技能指令与必要参数，例如 `Use lsou-store-diagnosis skill; period=week; dates=2026-09-07..2026-09-13; request=<account-scope>/<request-id>`。32 个业务 Skill 自行按编号读取本次资料与固定技能副本，按既定位置保存、回读并回传；消息不再展开本地路径、文件写入步骤或输出格式。此行为随新的源码版本打包交付，已冻结安装包保持原内容。
